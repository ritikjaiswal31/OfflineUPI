...
class IdempotencyConcurrencyTest {
    ...
    void singlePacketDeliveredByThreeBridgesSettlesExactlyOnce() throws Exception {
        ...
        BigDecimal aliceBefore = accounts.findById("node-1@upi").orElseThrow().getBalance();
        BigDecimal bobBefore = accounts.findById("node-2@upi").orElseThrow().getBalance();

        // One packet, but we'll deliver it from 3 "bridges" simultaneously
        MeshPacket packet = demoService.createPacket(
                "node-1@upi", "node-2@upi", new BigDecimal("100.00"), "1234", 5);
        ...
        BigDecimal aliceAfter = accounts.findById("node-1@upi").orElseThrow().getBalance();
        BigDecimal bobAfter = accounts.findById("node-2@upi").orElseThrow().getBalance();
    ...
    void tamperedCiphertextIsRejected() throws Exception {
        MeshPacket packet = demoService.createPacket(
                "node-1@upi", "node-2@upi", new BigDecimal("50.00"), "1234", 5);
    ...
    void encryptDecryptRoundTrip() throws Exception {
        PaymentInstruction original = new PaymentInstruction(
                "node-1@upi", "node-2@upi", new BigDecimal("123.45"),
